...
export function Nav() {
  ...
  const CenterIcon = isCliente
    ? Heart
    : isCerimonialista
      ? ShieldCheck
      : Briefcase;

  const secondHref = isFornecedor
    ? '/painel-fornecedor/fotos'
    : '/buscar';

  const secondLabel = isFornecedor
    ? 'Mídias'
    : 'Buscar';

  const SecondIcon = isFornecedor
    ? ImageIcon
    : Search;

  const fourthHref = isFornecedor
    ? '/painel-fornecedor/leads'
    : '/orcamentos';

  const fourthLabel = isFornecedor
    ? 'Leads'
    : 'Orçamentos';

  return (
    <>
      <div className="fixed right-5 top-5 z-[60] hidden max-w-[280px] items-start gap-3 rounded-[20px] bg-white/95 p-4 text-[#151515] shadow-2xl ring-1 ring-[#f1e7cf] backdrop-blur lg:flex">
        ...
        <div>
          ...
          <p className="mt-1 text-xs font-bold leading-5 text-gray-500">
            O REIM EVENTOS foi desenvolvido primeiro para uso no celular.
          </p>
        </div>
      </div>
