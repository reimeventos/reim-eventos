...
export default function CompartilharEventoPage() {
  ...
  async function handleRemove(id: string) {
    try {
      ...
      setErrorMessage(
        error?.message || 'Não foi possível remover o colaborador.'
      );
    } finally {
      setRemovingId('');
    }
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="mt-1 text-sm leading-5 text-gray-600">
                  A cerimonialista poderá colaborar no evento, ajudar na lista de
                  fornecedores e acompanhar os orçamentos.
                </p>
              </div>
            </div>
          </div>
          ...
          {!loading && collaborators.length === 0 && (
            <div className="rounded-[28px] bg-white p-6 text-center shadow-sm ring-1 ring-[#f1e7cf]">
              ...
              <p className="mt-2 text-sm leading-5 text-gray-500">
                Envie um convite para permitir colaboração no seu evento.
              </p>
            </div>
          )}
          ...
          <div className="space-y-4">
            {collaborators.map((item) => (
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <h3 className="text-lg font-extrabold">
                      {item.collaborator_name || 'Cerimonialista'}
                    </h3>
                ...
                >
                  <Trash2 size={17} />
                  {removingId === item.id ? 'Removendo...' : 'Remover convite'}
                </button>
              </div>
            ))}
          </div>
